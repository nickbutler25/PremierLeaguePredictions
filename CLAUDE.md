...
**OPEN 2026-08-29 — a wrong final score is permanent, because the first `FINISHED` is treated as final.** Liverpool 2-2 Nottingham Forest (GW2, external id 560552) sat in the UI as 1-1. The sync polled it at 13:36:16, read `FINISHED fullTime=1-1`, and wrote it. football-data finalised the record at `lastUpdated=13:42:10Z` with the correct `2-2` and `halfTime=0-1` — six minutes after our code had stopped looking. `SettledStatuses` and `NeedsPolling` (`ResultsService.cs:87,98`) drop a fixture from polling the moment it reads FINISHED, so nothing can ever revise it, and re-running the manual gameweek sync goes through the same filter and is equally blind.

**The free tier is delayed, not wrong — do not "fix" this by paying for live scores.** football-data's pricing page labels Free as "Scores delayed" (live scores start at €12/mo), and that delay is real: during the match the feed flapped IN_PLAY→TIMED five times serving stale pre-match records with `fullTime=null-null`, and `halfTime` stayed null for the full 90. But it settles to the truth ~20 minutes after full time. Verified by querying the free key directly. Volume is not a constraint either: Free has **no daily cap**, only 10 calls/min, against ~60 calls per match ≈ 600 per gameweek.

Four things to fix, none of which need a different provider or plan:
1. Keep polling past the first FINISHED — until `lastUpdated` stops moving, or ~30 min, which would have caught this with 24 minutes spare.
2. Reject backwards transitions (IN_PLAY/PAUSED/FINISHED → TIMED/SCHEDULED). Each flap rewrote the row, recalculated 22 picks and fired SignalR off a stale cache entry. `ResultsService.cs:159-160` already keeps the old score when scores are null; nothing guards the status.
3. Don't trust a `FINISHED` payload whose `halfTime` is null — that is an unfinalised record, and it is the tell that distinguishes the two.
4. `FixtureService.UpdateFixtureScoresAsync` (`FixtureService.cs:115-129`) writes the score and forces FINISHED but never recalculates points, invalidates standings, notifies clients or re-runs eliminations — so correcting a score by hand leaves the league table untouched.

Worth doing at the same time: poll `competitions/PL/matches?matchday=N` (all 10 fixtures, one call, confirmed working on the free key and returning live in-play data) instead of one `matches/{id}` per fixture. The saving is modest (~420 vs ~600 calls per gameweek) but it takes the peak burst from 10 calls in two seconds — right at the 10/min ceiling on a full simultaneous slate — down to 1.

Until this is fixed, the recovery path for a stuck score is `POST /api/v1/admin/sync/fixtures` (full-season sync, no settled filter — `FixtureSyncService.cs:248-260`) followed by `POST /api/v1/admin/gameweeks/{seasonId}/{n}/recalculate`, since the fixtures sync does not touch points. Standings are cached 5 minutes and neither call invalidates them.
