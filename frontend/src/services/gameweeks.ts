...
const realGameweeksService = {
  ...
  getCurrentGameweek: async (): Promise<Gameweek> => {
    const response = await apiClient.get<ApiResponse<Gameweek>>('/api/gameweeks/current');
    return response.data.data!;
  },

  getGameweekById: async (seasonId: string, weekNumber: number): Promise<Gameweek> => {
    const response = await apiClient.get<ApiResponse<Gameweek>>(`/api/gameweeks/${encodeURIComponent(seasonId)}/${weekNumber}`);
    return response.data.data!;
  },

  getPickRules: async (seasonId: string): Promise<PickRulesResponse> => {
    const response = await apiClient.get<ApiResponse<PickRulesResponse>>(`/api/gameweeks/pick-rules/${encodeURIComponent(seasonId)}`);
    return response.data.data!;
  },
};
