...
{
    Task OverridePickAsync(Guid pickId, int newTeamId, string reason, CancellationToken cancellationToken = default);
    Task RecalculatePointsForGameweekAsync(string seasonId, int gameweekNumber, CancellationToken cancellationToken = default);
    ...
    // Season management
    Task<string> CreateSeasonAsync(CreateSeasonRequest request, CancellationToken cancellationToken = default);
    ...
    Task UpdateTeamStatusAsync(int teamId, bool isActive, CancellationToken cancellationToken = default);
